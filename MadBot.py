                ...
                if self.units(CYBERNETICSCORE).ready.exists and self.can_afford(STALKER) and self.supply_left > 0:
                    ...
                    if self.build_order == 0:
                        ...
                        await self.chat_send(
                            "(glhf) MadBot v1.8: 2-Base Colossus BO chosen! Certainties: 2-Base Colossus: "
                        ...
                        print("--- One-Base Defend into DT BO chosen ---")
                        await self.chat_send(
                            "(glhf) MadBot v1.8: Rush-Defend into DT BO chosen! Certainties: 2-Base Colossus: "
                            + str(round(prediction[0][0] * 100, 2))
                            + "%; "
                            + "1-Base DTs: "
                            + str(round(prediction[0][1] * 100, 2))
                        ...
                        print("--- 4-Gate Proxy BO chosen ---")
                        await self.chat_send(
                            "(glhf) MadBot v1.8: 4-Gate Proxy BO chosen! Certainties: 2-Base Colossus: "
                            + str(round(prediction[0][0] * 100, 2))
                            + "%; "
                            + "1-Base DTs: "
                            + str(round(prediction[0][1] * 100, 2))
                        ...
                        print("--- 2-Base Adept/Immortal BO chosen ---")
                        await self.chat_send(
                            "(glhf) MadBot v1.8: 2-Base Adept/Immortal BO chosen! Certainties: 2-Base Colossus: "
                            + str(round(prediction[0][0] * 100, 2))
                            + "%; "
                            + "1-Base DTs: "
                            + str(round(prediction[0][1] * 100, 2))
                        ...
                        print("--- One-Base Defend into Voidrays BO chosen ---")
                        await self.chat_send(
                            "(glhf) MadBot v1.8: Rush-Defend into Voidrays BO chosen! Certainties: 2-Base Colossus: "
                            + str(round(prediction[0][0] * 100, 2))
                            + "%; "
                            + "1-Base DTs: "
                            + str(round(prediction[0][1] * 100, 2))
        ...
        for wg in self.units(WARPGATE).ready:
            ...
            if WARPGATETRAIN_ZEALOT in abilities:
                ...
                if self.units(ROBOTICSFACILITY).ready.exists and self.units(ROBOTICSFACILITY).ready.noqueue:
                    ...
                    self.units(CYBERNETICSCORE).ready.exists
                    and self.can_afford(SENTRY)
                    and (self.units(STALKER).amount + self.units(ADEPT).amount) / (self.units(SENTRY).amount + 1) > 6
                    and self.supply_left > 1
                ):
                    await self.do(wg.warp_in(SENTRY, warp_place))
                elif self.units(CYBERNETICSCORE).ready.exists and self.can_afford(STALKER) and self.supply_left > 1:
        ...
        if self.proxy_built and len(self.units(DARKTEMPLAR)) >= 1:
            if self.time > self.do_something_after and self.known_enemy_structures.exists:
                ...
                if all_enemy_base.exists and self.units(NEXUS).exists:
                    ...
                    attack_target_main = self.known_enemy_structures.closest_to(
                        self.units(DARKTEMPLAR).random.position
                    ).position.random_on_distance(random.randrange(1, 5))
